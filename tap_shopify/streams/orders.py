...
class Orders(Stream):
    ...
    def transform_discount_applications(self, data):
        ...
        Returns:
            list: List of discount applications.
        """
        discount_applications = [
            node for item in data["discountApplications"]["edges"]
        ...
        while page_info.get("hasNextPage"):
            params = {
                ...
                "discountafter": page_info.get("endCursor"),
            }

            # Fetch the next page of data
            response = self.call_api(params, query=query)
            applications_data = response.get("edges", [])[0].get("node", {})
            discount_applications.extend(
                node for item in applications_data["discountApplications"]["edges"]
                if (node := item.get("node"))
            )
            page_info = applications_data.get("pageInfo", {})
    ...
    def transform_object(self, obj):
        ...
        if obj.get("discountApplications"):
            obj["discountApplications"] = self.transform_discount_applications(obj)
        return obj
        ...
        return """
            ...
            orders(first: $first, after: $after, query: $query, sortKey: UPDATED_AT) {
                ...
                node {
                    ...
                    discountApplications(first: $first, after: $discountafter) {
                    edges {
                        node {
                        allocationMethod
                        index
